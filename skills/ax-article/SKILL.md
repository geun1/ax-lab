...
JSON 저장 후 **반드시 아래 두 명령을 연속 실행**합니다. 사용자에게 묻지 않고 바로 실행합니다:

```bash
node !`echo ${CLAUDE_SKILL_DIR}`/../../scripts/discord-send.js /tmp/ax-article-result.json
```

```bash
node !`echo ${CLAUDE_SKILL_DIR}`/../../scripts/db-save.js /tmp/ax-article-result.json
```
