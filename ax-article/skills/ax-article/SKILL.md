...
사용자가 제공한 콘텐츠를 분석하고, `/tmp/ax-article-result.json`에 Write하면 **로컬 Watcher가 자동으로 DB 저장 + Discord 전송**을 처리합니다.
...
분석이 완료되면 아래 JSON을 **Write 도구로 `/tmp/ax-article-result.json` 에 저장**합니다.
